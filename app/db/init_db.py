...
from app.models.entities import Admin, BotText, AppDownload, Setting, Product, Tutorial
...
def init_db():
    ...
    try:
        ...
        if not db.scalar(select(Tutorial).limit(1)): db.add(Tutorial(title="راهنمای اتصال", category="آموزش", content="ابتدا اپ را نصب و سپس کانفیگ را وارد کنید."))
        db.commit()
    finally:
        db.close()
