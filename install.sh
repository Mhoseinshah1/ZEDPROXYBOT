...
ensure_deps(){
  ...
  if ! compose_cmd >/dev/null 2>&1; then
    warn "docker compose plugin نصب نشد. تلاش برای نصب docker-compose قدیمی..."
    sudo apt-get install -y docker-compose || true
  fi

  compose_cmd >/dev/null 2>&1 || {
    err "Docker Compose نصب نشد."
    exit 1
  }
}
...
prompt_env(){
  ...
  read -rp "Admin username [main_admin]: " ADMIN_USERNAME
  ADMIN_USERNAME=${ADMIN_USERNAME:-main_admin}

  read -rsp "Admin password: " ADMIN_PASSWORD
  echo

  read -rp "دامنه اختیاری: " DOMAIN
  read -rp "فعال‌سازی SSL؟ yes/no [no]: " SSL_ENABLE
  ...
  read -rp "نام دیتابیس [zedproxy]: " DB_NAME
  DB_NAME=${DB_NAME:-zedproxy}

  read -rp "یوزر دیتابیس [zedproxy]: " DB_USER
  DB_USER=${DB_USER:-zedproxy}

  read -rsp "پسورد دیتابیس خالی=random: " DB_PASSWORD
  echo
  ...
  USE_WEBHOOK=false
  [[ -n "$DOMAIN" ]] && USE_WEBHOOK=true

  REPORT_GROUP_ENABLED=false
  [[ -n "$REPORT_GROUP_CHAT_ID" ]] && REPORT_GROUP_ENABLED=true

  WEB_BASE_URL=""
  [[ -n "$DOMAIN" ]] && WEB_BASE_URL="https://$DOMAIN"
...
run_migrate_seed(){
  ...
  $C exec -T app python -m app.db.init_db || true
}

health_check(){
  sleep 3
  if curl -fsS http://127.0.0.1:8000/health >/dev/null 2>&1; then
    msg "Health check موفق بود."
  else
    warn "Health check ناموفق بود. لاگ app و bot را بررسی کنید."
  fi
}

backup_db(){
  [[ -d "$INSTALL_DIR" ]] || {
    err "پروژه نصب نشده است."
    return
  }
  ...
  cd "$INSTALL_DIR"
  $C exec -T db pg_dump -U "${POSTGRES_USER:-zedproxy}" "${POSTGRES_DB:-zedproxy}" > "$file" 2>/dev/null || \
  $C exec -T db pg_dump -U zedproxy zedproxy > "$file"
...
restore_db(){
  ...
  read -rp "آیا مطمئن هستید؟ yes/no: " c
  [[ "$c" != "yes" ]] && return
  ...
  cd "$INSTALL_DIR"
  cat "$file" | $C exec -T db psql -U zedproxy -d zedproxy
...
install_robot(){
  ...
  if [[ ! -f "$INSTALL_DIR/.env" ]]; then
    ...
    msg ".env از قبل وجود دارد و بازنویسی نمی‌شود."
  fi
...
update_robot(){
  ...
  if [[ -f ".env" ]]; then
    mkdir -p "$BACKUP_DIR"
    cp .env "$BACKUP_DIR/env_before_update_$(date +%Y%m%d_%H%M%S)"
  fi
  ...
  git fetch origin || {
    err "git fetch شکست خورد."
    return
  }

  git pull origin main || {
    err "git pull شکست خورد."
    return
  }
...
remove_robot(){
  ...
  read -rp "آیا مطمئن هستید که می‌خواهید حذف کنید؟ yes/no: " c
  [[ "$c" != "yes" ]] && return
  ...
  read -rp "volumeها و دیتابیس هم حذف شوند؟ yes/no: " v
  [[ "$v" == "yes" ]] && $C down -v || true

  read -rp "مسیر پروژه هم حذف شود؟ yes/no: " d
...
while true; do
  echo
  msg "مدیریت ZedProxyBot"
  echo "1) نصب ربات"
  echo "2) آپدیت ربات"
  echo "3) حذف ربات"
  echo "4) بکاپ دیتابیس"
  echo "5) ری‌استور دیتابیس"
  echo "6) ری‌استارت سرویس‌ها"
  echo "7) مشاهده وضعیت سرویس‌ها"
  echo "8) مشاهده لاگ bot"
  echo "9) مشاهده لاگ app"
  echo "10) خروج"
